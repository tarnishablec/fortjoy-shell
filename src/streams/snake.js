...
export class SnakeGame {
  constructor(config) {
    ...
    this.snake$ = this.headPositionSub.pipe(
        outOfBoundP(this.config.tableSize, this),
  ...
  start() {
    this.subscriptions.push(this.step$.subscribe(this.headPositionSub));
    this.subscriptions.push(this.appleSub.subscribe(node => renderApple(node)));
    this.subscriptions.push(this.snake$.subscribe(nodes => renderSnake(nodes)));
  }
...
const outOfBoundP = (size,snake) => pipe(
    takeWhile(node => {
